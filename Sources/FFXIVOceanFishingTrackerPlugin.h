...
private:
	...
	struct contextMetaData_t
	{
		...
		std::string imageName; // the name of the image to use for this button
		...
		uint32_t skips = 0; // number of times to skip over, a skip=1 is "find next", skip=2 is "find next next", etc.
